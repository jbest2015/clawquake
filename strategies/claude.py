"""
Claude competitive strategy: Adaptive Predator v2.1.
"""
...
_LOG_PATH = os.path.join(os.getcwd(), 'results', 'claude_game.log')
_log_file = None
...
async def tick(bot, game, ctx):
    ...
    ctx.log_interval += 1
    if ctx.log_interval % 100 == 0:
        players = game.players if hasattr(game, 'players') else []
        _log(
            f"TICK={ctx.tick_count} hp={health} pos={my_pos} "
            f"players_visible={len(players)} falling={ctx.falling} "
            f"aggression={ctx.aggression} explore={ctx.explore_walk_timer}/{ctx.explore_walk_max}"
        )
    ...
    if ctx.engage_ticks == 1:
        ...
        _log(
            f"COMBAT target={enemy_name} dist={dist:.0f} hp={health} "
    ...
    if dist < 200:
        actions.append("weapon 3")
    elif dist < 600:
        actions.append("weapon 5")
    else:
        actions.append("weapon 4")
