"""
Quake 3 Huffman codec - pure Python implementation.

Uses a pre-computed tree (SAVED_TREE) from the Q3 engine's fixed frequency table.
The tree is stored as a flat array where:
  - Negative values (and zero) = leaf nodes, value is -symbol
  - Positive values = internal nodes, value is index of left child
    (right child is always value - 1)

Based on the approach from jfedor2/quake3-proxy-aimbot.
"""

NYT = 256  # Not Yet Transmitted sentinel


def _reverse_bit_order(value):
    """Reverse the bit order of a byte value."""
    return sum(((value & 1 << i) >> i) << (7 - i) for i in range(8))


class Node:
    __slots__ = ('symbol', 'parent', 'left', 'right', 'number', 'weight')
        ...
        while len(output) < length:
            if node.symbol is not None:
                if node.symbol == NYT:
                    ...
                    value = _reverse_bit_order(buffer.read_raw_bits(8))
                    output += bytes([value])
                    self.insert(value)
                else:
...
# Pre-computed tree from Q3 engine's fixed frequency table.
# Negative value (and zero) = leaf, value is -symbol.
# Positive value = internal node, value is index of left child.
# Right child is always value - 1.
SAVED_TREE = [
...
# Global fixed decoder instance - used for all Q3 network packet encoding/decoding
